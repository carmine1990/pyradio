...
def _writedotpylayers(typ,path):
    ...
    home = os.path.join(currentdir,'pyradio') #os.path.expanduser('~')
    with open(os.path.join(home,'.pyradio'),'a') as f:
        f.write(typ+'\n')
        f.write(path+'\n')
...
if os.path.isfile(os.path.join(home,'.pyradio')):
    os.remove(os.path.join(home,'.pyradio')) 
    _writedotpylayers('source',os.path.join(home,'pylayers-master'))
    _writedotpylayers('project',os.path.join(home,'pylayers_project'))


#if .pyradio exists
if os.path.isfile(os.path.join(home,'.pyradio')):
    with open(os.path.join(home,'.pyradio'),'r') as f:
        lines = f.readlines()
    #''.join... to remove the '\n' character

    pyradiodir = ''.join(lines[1].splitlines()) 
    print('pyradiodir:',pyradiodir)
    basename = ''.join(lines[3].splitlines()) 
    print("basename:", basename)

# BACKWARD COMPATIBILITY MODE (from now .pyradio is create each install)
else:
    # if os.getenv('PYLAYERS') != None:
    #     pylayersdir = os.getenv('PYLAYERS')
    #     _writedotpylayers('source',pylayersdir)
    #     print('PYLAYERS environement variable detected: ~/.pyradio updated')
    # else :
    ...
    #     print('BASENAME environement variable detected: ~/.pyradio updated')
    # else :
    #     raise EnvironmentError('pylayers source path not found. Try to re-run setup.py')
    raise EnvironmentError('pylayers source path not found.')
...
if basename != os.path.join(pyradiodir,'data'):
    if not 'win' in sys.platform:
    ...
    for dl in dirlist:
        filelist = os.listdir(os.path.join(pyradiodir,'data', dl))
        # print(os.path.join(pyradiodir,'data', dl))
        for fi in filelist:
            if not os.path.isdir(os.path.join(basename,dl,fi)):
                if os.path.isfile(os.path.join(basename,dl,fi)): # file already exists
                    ...
                    try:
                        shutil.copy(
                        os.path.join(pyradiodir,'data',dl,fi),
