...
class SRTMDownloader:
    ...
    def __init__(self, server="srtm.kurviger.de", #dwtkns.com/srtm30m/ #dds.cr.usgs.gov
                 directory=os.path.join('SRTM3').replace("\\","/"), #'srtm','version2_1','SRTM3'
                 cachedir="cache",
                 protocol="http"):
    ...
    def createFileListHTTP(self):
        ...
        url_srtm = self.protocol+'://'+self.server+'/'+self.directory
        print(url_srtm)
        if _PY3:
            req = Request(url_srtm, headers={'User-Agent': 'Mozilla/5.0'})
            r1 = urlopen(req)
        else:
            conn = urllib2.Request(url_srtm)
        ...
        for continent in continents:
            ...
            try:
                if _PY3:
                    req = Request(url_srtm+'/'+continent, headers={'User-Agent': 'Mozilla/5.0'})
                    r1 = urlopen(req)
                else:
                    conn = urllib2.Request(url_srtm+'/'+continent)
                    r1 = urllib2.urlopen(conn)
                data = r1.read()
                if isinstance(data,bytes):
                    data=data.decode('latin1')
                ...
                for filename in files:
                    self.filelist[self.parseFilename(filename)] = (
                                continent.replace("/index.html",""), filename)
            except:
                print("Download failed")
        
        #for filename in continents:
        #    self.filelist[self.parseFilename(filename)] = (
        #                filename, filename)
    ...
    def downloadTile(self, continent, filename):
        ...
        if self.protocol=="ftp":
            ...
            #Use HTTP
            if _PY3:
                req = Request(self.protocol+'://'+self.server+'/'+self.directory+'/'+continent+'/'+filename, headers={'User-Agent': 'Mozilla/5.0'})
                r1 = urlopen(req)
            else:
                conn = urllib2.Request(self.protocol+'://'+self.server+'/'+self.directory+'/'+continent+'/'+filename)
                r1 = urllib2.urlopen(conn)
            data = r1.read()
            self.ftpfile = open(os.path.join(self.cachedir,filename), 'wb')
