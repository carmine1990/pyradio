...
import pyradio.pyutil as pyu
#import pylayers.util.geomutil as geu
import pyradio.geomutil as geu
#import pylayers.util.plotutil as plu
import pyradio.plotutil as plu
#import pylayers.antprop.loss as loss
import pyradio.loss as loss
#from pylayers.util.project import *
from pyradio.project import *
from shapely.geometry import Polygon
#from pylayers.gis.gisutil import *
from pyradio.gisutil import *
#import pylayers.gis.srtm as srtm
import pyradio.srtm as srtm
...
class DEM(PyLayers):
    ...
    def loadraster(self,fileraster, box=[]):
        (lom,loM,lam,laM) = box
        with rasterio.open(fileraster) as raster:
            data = raster.read(1)
            self.hgta = data
            self.hgts = data
            self.extent = (lom,loM,lam,laM)
...
class Ezone(PyLayers):
    ...
    def rebase(self,source='srtm'):
        ...
        self.lat = np.linspace(self.extent[3],self.extent[2],Nlat)
        self.lonstep = (self.extent[1]-self.extent[0])/(Nlon-1.)
        #print('step lon:',self.lonstep)
        self.latstep = (self.extent[3]-self.extent[2])/(Nlat-1.)
        #print('step lan:',self.latstep)
    ...
    def tocart(self,Nx=1201,Ny=1201,source='srtm'):
        ...
        #  | y (axis 0)
        #  |
        #n = min(x.shape[0], y.shape[0])-1
        #lon,lat = self.m(x[None,:n],y[:n,None],inverse=True)
    ...
    def cover(self, **kwargs):
        ...
        if kwargs['source'] == 'aster':
            if rx.max() >self.hgta.shape[1]:
                rx = np.floor(( self.extent[1]-lon) / self.lonstep).astype(int)

            if ry.max() >self.hgta.shape[0]:
                ry = np.floor((lat - self.extent[2]) / self.latstep).astype(int)
...
#        return triang,LFS,L,Ltot
    ...
    def showcov(self, triang, val,
        ...
        extent = (lonmin, lonmax, latmin, latmax)
        #print(extent)
        mp = smopy.Map((extent[2]+0.015, extent[0]+0.015, extent[3]-0.015,extent[1]-0.015), z=12)
        if bbuild:
        ...
        cb.set_label('Loss(dB)',fontsize=18)
        return(cb)
